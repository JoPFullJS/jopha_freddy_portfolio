...
if(isset($_GET['categorie'])){
	...
	$count_categorie = $connect->prepare("
			SELECT  COUNT(c.slug) AS categorie,c.slug AS selected,c.titre,c.description,ct.id_categorie,ct.nom,ct.slug AS domaine 
	...
	if($count_categorie_fetch['categorie'] == 1){
		
		$categorie = $connect->prepare("
					...
					WHERE c.slug = :categorie AND t.affichage = 1
					ORDER BY t.id_travaux ASC");

		$categorie->bindValue('categorie', $_GET['categorie'], PDO::PARAM_STR);

		$categorie->execute();

		$categorie_fetch = $categorie->fetchAll();

		/*on selectionne les categorie qui appartiennent au doamine correspondant front, back ou ui*/

		$categories = $connect->prepare("
					SELECT c.nom,c.slug,c.titre 
					FROM competences AS c 
					WHERE c.travaux_key = 1  AND c.id_cat_compet = :categories 
					ORDER BY c.id_competences ASC");

		$categories->bindValue('categories', $count_categorie_fetch['id_categorie'], PDO::PARAM_STR);
...
<head>
	...
	<title><?= $count_categorie_fetch['titre']; ?></title>
	<meta name=" description" content="<?= $count_categorie_fetch['description']; ?>">
...
<section id="conteneur">
	...
	<header class="header-main-travaux header-general">
		...
		<div class="header-accroche">
			<h1><?= $count_categorie_fetch['titre']; ?></h1>
		</div>
	</header>
	...
	<div class="page-travaux article-section">
		...
		<div class="filter-categories">
			...
			<div class="choix-categorie">
				<select id="list-choix">
					<option value="<?= $count_categorie_fetch['domaine']; ?>">Travaux <?= $count_categorie_fetch['nom']; ?></option>
					<?php foreach ($categories_fetch as $k => $categorie): ?>
					<option value="<?= $categorie['slug']; ?>" <?php if($categorie['slug'] == $_GET['categorie']) echo "selected"; ?>><?= $categorie['nom']; ?></option>
					<?php endforeach; ?>
				</select>
			</div>
		</div>
		...
		<div class="group-travaux">
			...
			<?php foreach ($categorie_fetch as $k => $categorie): ?>
			<div class="list-travaux">

				<div class="info-user">
					<div class="image">
						<img src="<?= WEBROOT; ?>img/travaux/etiquettes/<?= $categorie['article']; ?>.jpg" alt="<?= $categorie['titre']; ?>">
						<a href="<?= WEBROOT; ?>travaux/front-end/<?= $categorie['categorie']; ?>/<?= $categorie['article']; ?>/" title="<?= $categorie['nom']; ?>" class="info-generales">
							<div class="info-glissante">
								<div class="techno">
									<img src="<?= WEBROOT; ?>img/competences/<?= $categorie['categorie']; ?>_small.png" alt="<?= $categorie['nom']; ?>">
								</div>
								...
								<div class="list-info">
										<div class="nb-vue">
											...
											<div class="info-vue">
												<p class="info1"><strong><?= $categorie['nb_vue']; ?></strong></p>
										...
										<?php 
											...
											$total_votes = $categorie['mediocre'] + $categorie['convenable'] + $categorie['bon'] + $categorie['tres_bon'] + $categorie['excellent'];
											...
											$moyenne_mediocre = ($categorie['mediocre'] / $total_votes) * $mediocre;
											$moyenne_convenable = ($categorie['convenable'] / $total_votes) * $convenable;
											$moyenne_bon = ($categorie['bon'] / $total_votes) * $bon;
											$moyenne_tres_bon = ($categorie['tres_bon'] / $total_votes) * $tres_bon;
											$moyenne_excellent = ($categorie['excellent'] / $total_votes) * $excellent;
				...
				<header class="titre-travaux">
					<p>
						<a href="<?= WEBROOT; ?>travaux/front-end/<?= $categorie['categorie']; ?>/<?= $categorie['article']; ?>/" title="<?= $categorie['titre']; ?>"><?= $categorie['titre']; ?></a>
					</p>
				</header>
